...
- 售后服务：读取可售后订单项、选择数量并提交原因、查看审核记录和详情。
- 限量活动：公开活动列表与详情、服务端时间校准倒计时、登录抢购、幂等重试、资格与异步订单结果查询。
- 品牌化体验：统一页面标题栏、悬浮底栏、桌面图标、启动图标、线性图标和浅色系统栏。
- 可访问性与稳定性：主要操作提供无障碍描述，写操作互斥，网络错误提供可重试状态，接口响应格式异常会被明确识别。

在线支付、物流跟踪和真实退款执行目前保留为未开放能力。售后审核通过仅代表审核状态，不虚构退款到账结果。
...
- 在线支付、物流和真实退款执行尚未接入；售后申请、审核记录和限量抢购已完成 App 端接口适配，但仍需真实服务端到端验收。
